...
import { RiMoneyDollarCircleFill,RiMoneyEuroCircleFill,RiMoneyPoundCircleFill } from "react-icons/ri";
...
const Content = () => {
  ...
  return (
    <section className="Content">
      ...
      {exchange.map((data) => {
        return (
          <Col xl={3} md={3} sm={3} xs={3} className="colContent" key={data.Name}>
              <Card className="cardPopular" style={{ width: "20rem" }}>
                <Row className="cardPopularTitleRow">
                  <Col xl={8} md={8} sm={8} xs={8}>
                    <Card.Title className="cardPopularTitle">{data.Name}</Card.Title>
                    <span className="cardPopularSpan">{data.Selling}</span>
                  </Col>
                  <Col xl={4} md={4} sm={4} xs={4} className="cardPopularCol">
                    <span>
                      {data.Name==="USD"? <RiMoneyDollarCircleFill className="cardTitleIcon"/>:data.Name==="EUR" ? <RiMoneyEuroCircleFill className="cardTitleIcon"/>:data.Name==="GBP" ?<RiMoneyPoundCircleFill className="cardTitleIcon"/>:<AiFillGolden className="cardTitleIcon"/>}
                    </span>
                  </Col>
                </Row>
                <Col>
                  <Card.Body>
                    <Card.Text>
                      <span className={(data.Change === "%0,00" || data.Change === "%-0,00") ? "text-warning" : data.Change.includes("-")  ? "text-danger" : "text-success"  }>
                        {(data.Change === "%0,00" || data.Change === "%-0,00") ? <AiOutlineLine /> : data.Change.includes("-")  ? <FaArrowDown/> : <FaArrowUp/>  }{data.Change}
                      </span>
                    </Card.Text>
                  </Card.Body>
                </Col>
